...
from consulta_datos import start_consult_data,datos_handler
from consulta_datos import RANGO_FECHA
...
async def button(update: Update, context: CallbackContext) -> None:
    ...
    if  user_reply  == 'Reparación 🛠️':
        ...
        await update.message.reply_text(text="¡Te comparto las estadisticas!",reply_markup=ReplyKeyboardRemove())
        await start_consult_data(update,context)
        return RANGO_FECHA
        #return ConversationHandler.END
...
def main():
    ...
    # Configuración del ConversationHandler para manejar la consulta de datos estadisticos
    convData_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex('^📊$'), button)],
        states={
            RANGO_FECHA: [MessageHandler(filters.TEXT & ~filters.COMMAND, datos_handler)]
        },
        fallbacks=[CallbackQueryHandler(button_regcancel, pattern='^cancel$')],
        #fallbacks=[MessageHandler(filters.Regex('^cancel$'), button)]
    )
    app.add_handler(convData_handler)
